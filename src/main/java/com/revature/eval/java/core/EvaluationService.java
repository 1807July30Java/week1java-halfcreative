...
public class EvaluationService {
	...
	public String acronym(String phrase) {
		...
		// make an array of characters that is equal to the length of words in the
		// phrase given
	...
	static class Triangle {
		...
		public boolean isScalene() {
			// we could just check if its neither a scalene or isoscelese by the rules given
			// but hey, lets be legit
			if (this.sideOne != this.sideTwo && this.sideOne != this.sideThree && this.sideTwo != this.sideThree) {
				return true;
			} else {
				return false;
	...
	public int getScrabbleScore(String string) {
		...
		for (String letter : letters) {
			String uLetter = letter.toUpperCase(); // get uppercase, so we dont have to worry about case
			switch (uLetter) {
			case "D":
			case "G":
				pointVal += 2;
	...
	public String toPigLatin(String string) {
		...
		for (int i = 0; i < words.length; i++) {
			do {
				...
				case 'y':
				case 'z':
					String wordy = words[i].substring(1);
					words[i] = wordy + words[i].charAt(0);
				}
